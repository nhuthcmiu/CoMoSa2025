<!DOCTYPE html>
<html lang=en xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:m="http://schemas.microsoft.com/office/2004/12/omml">
<?php include('head.html');?>
<body>
<header>
<?php include('menu.html');?>
<style>
    table {
      width: 100%;
      border-collapse: collapse;
      margin-top: 30px;
    }
    th, td {
      border: 1px solid #444;
      padding: 15px;
      vertical-align: top;
    }
    th {
      background-color: #f2f2f2;
      text-align: center;
    }
    ul {
      margin: 0;
      padding-left: 20px;
    }
    li {
      margin-bottom: 8px;
    }
  </style>
</header>
<div class="twelve columns section">
<div class=container>
<h1>Program</h1>
</div>
</div>
<div class="wrap">
  <div class="tc">
  <h4><strong>Registration link:</strong> To Be Announced</h4>

    <h3>Registration rate:</h3>
	
    <table>
        <thead>
            <tr>
                <th>Category</th>
                <th>Before 15 July 2025</th>
                <th>After 15 July 2025</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>Registration (Author or Non-Author)</td>
                <td> USD</td>
                <td> USD</td>
            </tr>
            <tr>
                <td>Full Time Student</td>
                <td> USD</td>
                <td> USD</td>
            </tr>
        </tbody>
    </table>

    <h3>Important information:</h3>
    <ul>
		<li>-At least one author of each accepted paper must register and pay full registration fee () by ../../ 2025.</li>
		<li>-All registrations associated with accepted papers should be made before ../../ 2025.</li>
		<li>-If you have more than one accepted paper, you have to register for each one individually.</li>
		<li>-The registration fee includes admission to all conference sessions, conference proceedings, coffee break refreshments, lunches and gala dinner.</li>
		<li>-For other registration (non-authors, co-authors, or full time students not registering a paper), a --- handling fee will be applied if the cancellation is before ../../ 2025.</li>
		<li>-Fees cannot be refunded for registrations canceled after ../../ 2025.</li>
      </ul>

    <h3>Financial support:</h3>
    <p>-Authors with papers conducted in Vietnamese universities can apply for financial support by sending an email to <a href="mailto:comosa2025@vinuni.edu.vn">comosa2025@vinuni.edu.vn</a> from the email used in paper submission. The email must include paper ID, paper title, and author’s affiliation.</p>
    <p>-We also have some limited support for non-authors from Vietnamese universities (especially students). Email to <a href="mailto:comosa2025@vinuni.edu.vn">comosa2025@vinuni.edu.vn</a> for more details.</p>


  
  </div>
  
</div>
<?php include('foot.html');?>
</body>
</html>